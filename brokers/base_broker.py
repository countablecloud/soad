...
from database.models import AccountInfo, Balance, Position, Trade
...
class BaseBroker(ABC):
    ...
    async def is_order_filled(self, order_id):
        ...
        try:
            ...
            logger.error(
                "Failed to check if order has been filled", extra={"error": str(e)}
            )
            return False
    ...
    async def get_account_info(self):
        ...
        try:
            ...
            await self.db_manager.add_account_info(
                AccountInfo(broker=self.broker_name, value=account_info["value"])
            )
            logger.debug(
                "Account information retrieved", extra={"account_info": account_info}
            )
            return account_info
        except Exception as e:
    ...
    async def update_positions(self, trade_id, session):
        ...
        try:
            ...
            if trade.quantity == 0:
                logger.error(
                    "Trade quantity is 0, doing nothing", extra={"trade": trade}
                )
                return
            ...
            # Handling Sell Orders
                ...
                if position:
                    ...
                    profit_loss = (
                        float(trade.executed_price) - cost_per_share
                    ) * trade.quantity
                    logger.info(
                    ...
                    if position.quantity == trade.quantity:  # Full sell
                        logger.info(
                            "Deleting sold position", extra={"position": position}
                        )
                        await session.delete(position)
                    else:  # Partial sell
    ...
    async def place_future_option_order(
        self, symbol, quantity, side, strategy, price=None, order_type="limit"
    ):
        ...
        return await self._place_order_generic(
            ...
            price,
            multiplier,
            self._place_future_option_order,
            order_type,
        )
    ...
    async def _place_order_generic(
        ...
        if (
            self.prevent_day_trading
            and side == "sell"
            and await self.has_bought_today(symbol)
        ):
            logger.error(
        ...
        try:
            if asyncio.iscoroutinefunction(broker_order_func):
                response = await broker_order_func(
            ...
            async with self.Session() as session:
                ...
                latest_balance = await session.execute(
                    select(Balance)
                    .filter_by(broker=self.broker_name, strategy=strategy, type="cash")
                ...
                if latest_balance:
                    ...
                    new_balance_amount = (
                        latest_balance.balance - order_cost
                        if side == "buy"
                        else latest_balance.balance + order_cost
                    )
        ...
        try:
            ...
            async with self.Session() as session:
                trade = await session.execute(
                    session.query(Trade).filter_by(id=order_id)
                )
    ...
    async def cancel_order(self, order_id):
        ...
        try:
            ...
            logger.info(
                "Order cancelled successfully", extra={"cancel_status": cancel_status}
            )
    ...
    def get_options_chain(self, symbol, expiration_date):
        ...
        logger.info(
            ...
            extra={"symbol": symbol, "expiration_date": expiration_date},
        )
        try:
            options_chain = self._get_options_chain(symbol, expiration_date)
            logger.info(
                "Options chain retrieved", extra={"options_chain": options_chain}
            )
