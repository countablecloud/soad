...
from soad.utils.utils import extract_underlying_symbol
...
class TradierBroker(BaseBroker):
    ...
    def _place_order(self, symbol, quantity, side, price=None, order_type="limit"):
        ...
        try:
            ...
            if order_type == "limit":
                ...
                logger.error(
                    "Invalid order type",
                    extra={"order_type": order_type, "symbol": symbol},
                )
                return
    ...
    def _place_future_option_order(self, symbol, quantity, side, price=None):
        logger.error(
            "Future options not supported by Tradier", extra={"symbol": symbol}
        )
    ...
    def _get_options_chain(self, symbol, expiration_date):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/markets/options/chains?symbol={symbol}&expiration={expiration_date}",
                headers=self.headers,
            )
            response.raise_for_status()
    ...
    async def get_current_price(self, symbol):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.base_url}/markets/quotes?symbols={symbol}",
                    headers=self.headers,
                ) as response:
                    response.raise_for_status()
                    data = await response.json()
    ...
    def get_bid_ask(self, symbol):
        ...
        try:
            ...
            ask = quote.get("ask")
            logger.info(
                "Bid/ask retrieved", extra={"symbol": symbol, "bid": bid, "ask": ask}
            )
