...
from tastytrade.instruments import Equity, FutureOption, NestedOptionChain, Option
...
from soad.utils.utils import extract_underlying_symbol, is_futures_symbol, is_option
...
class TastytradeBroker(BaseBroker):
    ...
    async def _place_future_option_order(
        self, symbol, quantity, side, price=None, order_type="limit"
    ):
        ticker = extract_underlying_symbol(symbol)
        logger.info(
            "Placing future option order",
            extra={
                "symbol": symbol,
                "quantity": quantity,
                "side": side,
        ...
        if price is None:
            ...
            logger.info(
                "Price not provided, using mid from current bid/ask",
                extra={"price": price},
            )
        if side == "buy":
    ...
    async def _place_order(
        ...
        try:
            ...
            if getattr(response, "errors", None):
                logger.error(
                    "Order placement failed with no order ID",
                    extra={
    ...
    def _is_order_filled(self, order_id):
        ...
        all_legs_filled = all(
            [
                leg.get("remaining-quantity") == 0
                for leg in status.get("data").get("legs")
            ]
        )
    ...
    def _cancel_order(self, order_id):
        ...
        try:
            ...
            logger.info(
                ...
                extra={"cancellation_response": cancellation_response},
            )
    ...
    async def get_current_price(self, symbol):
        if ":" in symbol:
            ...
            logger.info(
                "Getting current price for futures symbol", extra={"symbol": symbol}
            )
            option = FutureOption.get_future_option(self.session, symbol)
