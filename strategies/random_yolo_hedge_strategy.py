...
class RandomYoloHedge(BaseStrategy):
    ...
    async def rebalance(self):
        ...
        with self.broker.Session() as session:
            ...
            for position in positions:
                ...
                # TODO: THIS ISNT ACTUALLY CHECKING?
                _ = (
                    session.query(Trade)
                    .filter_by(
                        broker=self.broker.broker_name,
        ...
        if valid_call_option and valid_put_option:
            ...
            await self.place_option_order(
                valid_call_option["symbol"],
                call_bet_size // valid_call_option["lastPrice"],
                "buy",
                valid_call_option,
            )
            await self.place_option_order(
    ...
    async def find_valid_option(self, stocks, option_type, total_balance):
        ...
        exp_date = (
            current_date + timedelta(days=(4 - current_date.weekday()))
        ).strftime("%Y-%m-%d")
        while stocks:
            stock = random.choice(stocks)
            stocks.remove(stock)
            option = await self.get_atm_option(stock, exp_date, option_type)
            if option and self.is_order_valid(
                option, total_balance * self.bet_percentage * 0.5
            ):
                return option
            else:
                logger.info(
                    f"Invalid {option_type} option for {stock}, trying another stock."
                )
