...
from soad.utils.utils import is_market_open
...
class BlackSwanStrategy(BaseStrategy):
    ...
    async def rebalance(self):
        ...
        with self.broker.Session() as session:
            ...
            if balance is None:
                logger.error(
                    f"Strategy balance not initialized for {self.strategy_name} "
                    f"strategy on {self.broker.broker_name}."
                )
                raise ValueError(
                    f"Strategy balance not initialized for {self.strategy_name} "
                    f"strategy on {self.broker.broker_name}."
        ...
        valid_put_option = await self.find_valid_option(
            self.symbol, "put", total_balance
        )
        ...
        if valid_put_option:
            ...
            await self.place_option_order(
                valid_put_option["symbol"],
                put_bet_size // valid_put_option["lastPrice"],
                "buy",
                valid_put_option,
            )

    async def handle_previous_positions(
        self, current_db_positions_dict, previous_trades
    ):
        ...
        for position, details in current_db_positions_dict.items():
            ...
            buy_price = next(
                (trade.price for trade in previous_trades if trade.symbol == position),
                None,
            )
            if not buy_price:
                continue
            ...
            if (
                days_held >= self.holding_period_days
                or (current_price / buy_price - 1) * 100 >= self.spike_percentage
            ):
                await self.broker.close_position(
                    position, details["quantity"], self.strategy_name
                )
    ...
    async def find_valid_option(self, symbol, option_type, total_balance):
        ...
        closest_exp_date = min(
            exp_dates, key=lambda x: abs(x - target_exp_date)
        ).strftime("%Y-%m-%d")
        ...
        if option and self.is_order_valid(option, total_balance * self.bet_percentage):
            ...
            logger.info(
                f"Invalid {option_type} option for {symbol}, no valid option found."
            )
            return None
    ...
    async def get_otm_option(self, symbol, exp_date, option_type):
        ...
        current_price = (
            await self.broker.get_current_price(symbol)
            if asyncio.iscoroutinefunction(self.broker.get_current_price)
            else self.broker.get_current_price(symbol)
        )
        ...
        otm_options = options[
            options["strike"] < current_price * (1 - self.otm_percentage)
        ]
    ...
    def is_order_valid(self, option, bet_size):
        ...
        if spread / last_price > self.max_spread_percentage:
            logger.error(
                f"Order for {option['symbol']} rejected due to high spread: {spread / last_price:.2%}"
            )
            return False

        if last_price > bet_size:
            logger.error(
                f"Order for {option['symbol']} rejected due to high price: {last_price} > {bet_size}"
            )
            return False
    ...
    async def place_option_order(self, symbol, quantity, side, option):
        ...
        if self.paper_trade:
            logger.info(
                f"Paper trading: Placed {side} order for {symbol}: {quantity} shares at {price}",
                extra={"strategy_name": self.strategy_name},
            )
            self.broker.place_option_order(
                symbol, quantity, side, self.strategy_name, price, paper_trade=True
            )
            return
        await self.broker.place_option_order(
            symbol, quantity, side, self.strategy_name, price
        )
