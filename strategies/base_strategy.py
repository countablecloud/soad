...
from soad.utils.utils import is_futures_market_open, is_futures_symbol, is_market_open


class BaseStrategy(ABC):
    def __init__(
        self, broker, strategy_name, starting_capital, rebalance_interval_minutes=5
    ):
        self.broker = broker
    ...
    async def initialize_starting_balance(self):
        if self.initialized:
            logger.debug(
                "Starting balance already initialized",
                extra={"strategy_name": self.strategy_name},
            )
            return
        logger.debug(
            "Initializing starting balance", extra={"strategy_name": self.strategy_name}
        )

        account_info = await self.broker.get_account_info()
        buying_power = account_info.get("buying_power")
        logger.debug(
            f"Account info: {account_info}", extra={"strategy_name": self.strategy_name}
        )

        if buying_power < self.starting_capital:
            logger.error(
                f"Not enough cash available. Required: {self.starting_capital}, Available: {buying_power}",
                extra={"strategy_name": self.strategy_name},
            )
            raise ValueError(
        ...
        async with self.broker.Session() as session:
            ...
            if strategy_balance is None:
                ...
                logger.info(
                    (
                        f"Initialized starting balance for {self.strategy_name} "
                        f"strategy with {self.starting_capital}"
                    ),
                    extra={"strategy_name": self.strategy_name},
                )
            else:
                logger.info(
                    f"Existing balance found for {self.strategy_name} strategy: {strategy_balance.balance}",
                    extra={"strategy_name": self.strategy_name},
                )
        ...
        async with self.broker.Session() as session:
            result = await session.execute(
                select(Position).filter_by(
                    strategy=self.strategy_name, broker=self.broker.broker_name
                )
            )
    ...
    async def sync_positions_with_broker(self):
        logger.debug(
            "Syncing positions with broker", extra={"strategy_name": self.strategy_name}
        )

        broker_positions = self.broker.get_positions()  # Assuming this is synchronous
        logger.debug(
            f"Broker positions: {broker_positions}",
            extra={"strategy_name": self.strategy_name},
        )
        ...
        async with self.broker.Session() as session:
            for symbol, data in broker_positions.items():
                if is_futures_symbol(symbol):
                    logger.info(
                        f"Skipping syncing positions for futures option {symbol}",
                        extra={"strategy_name": self.strategy_name},
                    )
                    continue
                ...
                if target_quantity is not None and target_quantity > 0:
                    ...
                    if position:
                        ...
                        # Update the position with the minimum of the target quantity
                        # and the broker's quantity
                        ...
                        logger.info(
                            (
                                f"Updated position for {symbol} with quantity "
                                f"{position.quantity} and price {current_price}"
                            ),
                            extra={"strategy_name": self.strategy_name},
                        )
                    else:
                        ...
                        logger.info(
                            (
                                f"Created new position for {symbol} with quantity "
                                f"{data['quantity']} and price {current_price}"
                            ),
                            extra={"strategy_name": self.strategy_name},
                        )
                    ...
                    if target_quantity < data["quantity"]:
                        ...
                        logger.info(
                            (
                                f"Created uncategorized position for {symbol}"
                                f"with quantity {data['quantity'] - target_quantity} and "
            ...
            for position in db_positions:
                if position.symbol not in broker_symbols:
                    logger.info(
                        f"Removing position for {position.symbol} as it's not in broker's positions",
                        extra={"strategy_name": self.strategy_name},
                    )
    ...
    def calculate_target_balances(self, total_balance, cash_percentage):
        ...
        logger.debug(
            (
                f"Target cash balance: {target_cash_balance}, "
                f"Target investment balance: {target_investment_balance}"
            ),
            extra={"strategy_name": self.strategy_name},
        )
    ...
    async def fetch_current_db_positions(self):
        ...
        logger.debug(
            f"Current DB positions: {current_db_positions_dict}",
            extra={"strategy_name": self.strategy_name},
        )
        return current_db_positions_dict

    async def place_future_option_order(
        self, symbol, quantity, side, price, wait_till_open=True, order_type="limit"
    ):
        if is_futures_market_open() or not wait_till_open:
            await self.broker.place_future_option_order(
                symbol, quantity, side, self.strategy_name, price, order_type
            )
            logger.info(
                f"Placed {side} order for {symbol}: {quantity} shares",
                extra={
            ...
            logger.info(
                f"Market is closed, not placing {side} order for {symbol}: {quantity} shares",
                extra={
                    "strategy_name": self.strategy_name,
                    "symbol": symbol,
                    "quantity": quantity,
    ...
    async def place_order(
        self, stock, quantity, side, price, wait_till_open=True, order_type="limit"
    ):
        if is_market_open() or not wait_till_open:
            await self.broker.place_order(
                stock, quantity, side, self.strategy_name, price, order_type
            )
            logger.info(
                f"Placed {side} order for {stock}: {quantity} shares",
                extra={
                    "strategy_name": self.strategy_name,
                    "stock": stock,
                    "quantity": quantity,
            ...
            logger.info(
                f"Market is closed, not placing {side} order for {stock}: {quantity} shares",
                extra={
                    "strategy_name": self.strategy_name,
                    "stock": stock,
                    "quantity": quantity,
