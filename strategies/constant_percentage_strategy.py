...
class ConstantPercentageStrategy(BaseStrategy):
    ...
    async def rebalance(self):
        ...
        target_cash_balance, target_investment_balance = self.calculate_target_balances(
            total_balance, self.cash_percentage
        )
        ...
        for stock, allocation in self.stock_allocations.items():
            ...
            current_price = (
                await self.broker.get_current_price(stock)
                if asyncio.iscoroutinefunction(self.broker.get_current_price)
                else self.broker.get_current_price(stock)
            )
            ...
            if current_position < target_quantity * (1 - self.buffer):
                await self.place_order(
                    stock, target_quantity - current_position, "buy", current_price
                )
            # If we own more than the target quantity plus or minus the buffer, sell the excess
                ...
                await self.place_order(
                    stock, current_position - target_quantity, "sell", current_price
                )
