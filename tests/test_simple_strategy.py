...
async def test_should_buy():
    ...
    assert await strategy.should_buy("AAPL", 90) is True
    assert await strategy.should_buy("AAPL", 110) is False
...
async def test_should_sell():
    ...
    assert await strategy.should_sell("AAPL", 160) is True
    assert await strategy.should_sell("AAPL", 140) is False
