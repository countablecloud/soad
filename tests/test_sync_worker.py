...
async def test_update_position_prices_and_volatility():
    ...
    mock_broker_instance.get_cost_basis = MagicMock(
        return_value=100.0
    )  # Synchronous function

    # Mock get_broker_instance to return the mock broker instance
    mock_broker_service.get_broker_instance = AsyncMock(
        return_value=mock_broker_instance
    )
    ...
    await position_service.update_position_prices_and_volatility(
        mock_session, mock_positions, timestamp
    )
...
async def test_update_strategy_balance(mock_logger, balance_service):
    ...
    await balance_service.update_strategy_balance(
        mock_session, "mock_broker", "strategy1", datetime.now()
    )
    assert (
        mock_session.add.called
    )  # Check that session.add was called to add a new balance record
    assert mock_session.commit.called
...
async def test_update_uncategorized_balances(mock_logger, balance_service):
    mock_session = AsyncMock(AsyncSession)
    balance_service.broker_service.get_account_info = AsyncMock(
        return_value={"value": 1000}
    )
    balance_service._sum_all_strategy_balances = AsyncMock(return_value=800)
    await balance_service.update_uncategorized_balances(
        mock_session, "mock_broker", datetime.now()
    )
...
async def test_get_positions(position_service):
    ...
    position_service.broker_service.get_broker_instance.return_value = (
        mock_broker_instance
    )

    position_service._fetch_db_positions = AsyncMock(return_value={})

    broker_positions, db_positions = await position_service._get_positions(
        mock_session, "mock_broker"
    )
...
async def test_insert_new_position():
    ...
    mock_broker_position = {"symbol": "AAPL", "quantity": 10, "latest_price": 150.0}
    # Create PositionService and timestamp
    position_service = PositionService(AsyncMock())
    now = datetime.now()

    # Call the method
    await position_service._insert_new_position(
        mock_session, "mock_broker", mock_broker_position, now
    )
...
async def test_insert_or_update_balance(mock_logger, balance_service):
    mock_session = AsyncMock(AsyncSession)
    await balance_service._insert_or_update_balance(
        mock_session, "mock_broker", "strategy1", 1000, datetime.now()
    )
...
async def test_fetch_and_update_positions(mock_logger):
    ...
    await _fetch_and_update_positions(
        mock_session, mock_position_service, datetime.now()
    )

    mock_session.execute.assert_called_once_with(ANY)
    # mock_position_service.update_position_prices_and_volatility.assert_awaited_once_with(
    # mock_session, mock_positions.scalars(), ANY)
...
async def test_reconcile_brokers_and_update_balances(mock_logger):
    ...
    mock_balance_service.update_all_strategy_balances.assert_any_await(
        mock_session, "broker1", mock_now
    )
    mock_balance_service.update_all_strategy_balances.assert_any_await(
        mock_session, "broker2", mock_now
    )
    data.sync_worker.RECONCILE_POSITIONS = False
...
async def test_update_strategy_and_uncategorized_balances():
    ...
    mock_broker_service.get_latest_price.side_effect = lambda broker, symbol: (
        100 if symbol == "AAPL" else 200 if symbol == "GOOGL" else 150
    )
    ...
    await balance_service.update_all_strategy_balances(
        mock_session, "tradier", datetime.now()
    )
...
async def test_insert_or_update_position():
    ...
    mock_broker_position_new = {"symbol": "MSFT", "quantity": 20, "latest_price": 200.0}
    ...
    await position_service._insert_new_position(
        mock_session, "mock_broker", mock_broker_position_existing, now
    )
    ...
    await position_service._insert_new_position(
        mock_session, "mock_broker", mock_broker_position_new, now
    )
...
async def test_remove_excess_uncategorized_positions_no_categorized():
    ...
    broker_positions = {"AAPL": {"quantity": 80}}

    # Create PositionService instance
    position_service = PositionService(AsyncMock())

    # Call the function
    await position_service._remove_excess_uncategorized_positions(
        mock_session, "mock_broker", db_positions, broker_positions
    )
...
async def test_remove_excess_uncategorized_positions_no_change_needed():
    ...
    broker_positions = {
        "AAPL": {
            "quantity": 70
        }  # Broker position matches DB (50 uncategorized + 20 categorized)
    }
...
async def test_remove_excess_uncategorized_positions_negative_net_broker_quantity():
    ...
    # Since categorized quantity (90) exceeds broker quantity (80),
    # uncategorized position should be set to 0
...
async def test_run_sync_worker_iteration_timeout(mock_logger):
    ...
    mock_logger.error.assert_any_call(
        "Iteration exceeded the maximum allowed time. Forcing restart."
    )

    # Ensure reconcile and update methods were attempted
    mock_position_service.reconcile_positions.assert_called_once_with(
        mock_session, "mock_broker"
    )
    # Should not reach this due to timeout
    mock_balance_service.update_all_strategy_balances.assert_not_called()
    data.sync_worker.RECONCILE_POSITIONS = False
