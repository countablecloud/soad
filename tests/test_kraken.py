...
class TestKrakenBroker(BaseTest):
    ...
    async def test_get_account_info(self, mock_post):
        ...
        mock_response.json.return_value = {
            "result": {"ZUSD": "10000.0000", "XXBT": "1.0000"}
        }
        mock_post.return_value = mock_response

        await self.broker.connect()
        account_info = await self.broker._get_account_info()
        assert account_info == {"ZUSD": "10000.0000", "XXBT": "1.0000"}
    ...
    async def test_get_positions(self, mock_post):
        mock_response = MagicMock()
        mock_response.json.return_value = {
            "result": {"TXID1": {"pair": "XXBTZUSD", "type": "buy", "volume": "1.0000"}}
        }
        mock_post.return_value = mock_response
        ...
        assert positions == {
            "XXBTZUSD": {"pair": "XXBTZUSD", "type": "buy", "volume": "1.0000"}
        }
    ...
    async def test_place_order(self, mock_post):
        ...
        mock_response.json.return_value = {
            "result": {
                "txid": ["TXID1"],
                "descr": {"order": "buy 1.00000000 XBTUSD @ limit 50000.0"},
            }
        }
        mock_post.return_value = mock_response
        ...
        balance = await self.session.execute(
            select(Balance).filter_by(broker="Kraken", strategy="default")
        )
    ...
    async def test_cancel_order(self, mock_post):
        mock_response = MagicMock()
        mock_response.json.return_value = {"result": {"count": 1}}
        mock_post.return_value = mock_response
    ...
    async def test_get_bid_ask(self, mock_get):
        ...
        mock_response.json.return_value = {
            "result": {
                "XXBTZUSD": {"b": ["49900.0", "1", "1.0"], "a": ["50000.0", "1", "1.0"]}
            }
        }
    ...
    async def test_get_current_price(self, mock_get):
        ...
        mock_response.json = MagicMock(
            return_value={"result": {"XXBTZUSD": {"c": ["50000.0", "1.00000000"]}}}
        )
...
def skip_test_get_account_info_usd_balance(mock_make_request, kraken_broker):
    ...
    mock_make_request.return_value = {"result": {"ZUSD": "1000.00"}}
