...
@pytest.mark.asyncio
@patch(
    "strategies.constant_percentage_strategy.ConstantPercentageStrategy.calculate_target_balances"
)
...
async def skip_test_rebalance(
    mock_is_market_open,
    mock_current_db_positions,
    mock_should_own,
    mock_calculate_target_balances,
    strategy_setup,
):
    ...
    # Mock stock prices
    mock_broker.get_current_price.side_effect = lambda symbol: (
    ...
    mock_broker.place_order.assert_any_call(
        "AAPL", 15 - 10, "buy", "constant_percentage", 100
    )
    mock_broker.place_order.assert_any_call(
        "GOOGL", 10 - 5, "buy", "constant_percentage", 200
    )
    mock_broker.place_order.assert_any_call(
        "MSFT", 15 - 10, "sell", "constant_percentage", 150
    )
...
@patch("strategies.base_strategy.BaseStrategy.fetch_current_db_positions")
@patch("strategies.base_strategy.is_market_open")
async def skip_test_no_rebalance_needed(
    mock_is_market_open,
    mock_current_db_positions,
    mock_should_own,
    mock_calculate_target_balances,
    ...
    mock_current_db_positions.return_value = {"AAPL": 15, "GOOGL": 10, "MSFT": 10}

    # Mock should_own return values
    mock_should_own.side_effect = [
        15,
        10,
        10,
    ]  # Target quantities: AAPL(10), GOOGL(5), MSFT(15)
...
@patch("strategies.base_strategy.BaseStrategy.fetch_current_db_positions")
@patch("strategies.base_strategy.is_market_open")
async def skp_test_no_rebalance_needed(
    mock_is_market_open,
    mock_current_db_positions,
    mock_should_own,
    mock_calculate_target_balances,
    ...
    mock_is_market_open.return_value = True  # Market is open
    # Current positions match target
    mock_current_db_positions.return_value = {"AAPL": 15, "GOOGL": 10, "MSFT": 51}
    ...
    # Ensure that no orders were placed since the target is already met
    mock_broker.place_order.assert_any_call(
        "MSFT", 41, "sell", "constant_percentage", 150
    )
...
@patch("strategies.base_strategy.BaseStrategy.fetch_current_db_positions")
@patch("strategies.base_strategy.is_market_open")
async def skip_test_rebalance_with_custom_buffer(
    mock_is_market_open,
    mock_current_db_positions,
    mock_should_own,
    mock_calculate_target_balances,
    ...
    mock_should_own.side_effect = [
        ...
        4,
        14,
    ]  # Slightly different target quantities: AAPL(12), GOOGL(4), MSFT(14)
...
@patch("strategies.base_strategy.BaseStrategy.fetch_current_db_positions")
@patch("strategies.base_strategy.is_market_open")
async def skip_test_rebalance_with_custom_buffer_control(
    mock_is_market_open,
    mock_current_db_positions,
    mock_should_own,
    mock_calculate_target_balances,
    ...
    # Since the buffer is 20%, and all positions are within the buffer, no orders should be placed
    mock_broker.place_order.assert_any_call(
        "MSFT", 2, "sell", "constant_percentage", 150
    )
