...
class TestTradierBroker(BaseTest):
    ...
    async def test_place_order(self, mock_get, mock_post):
        ...
        mock_post.return_value = MagicMock(
            json=MagicMock(return_value={"status": "filled", "filled_price": 155.00})
        )

        await self.broker.connect()
        await self.broker.get_account_info()
        order_info = await self.broker.place_order(
            "AAPL", 10, "buy", "example_strategy", 150.00
        )
        ...
        balance = await self.session.execute(
            select(Balance).filter_by(broker="E*TRADE", strategy="example_strategy")
        )
        balance = balance.scalar_one()
        assert balance is not None
        assert balance.total_balance == 10000.0 + (
            10 * 155.00
        )  # Assuming the balance should include the executed trade
