...
from soad.ui.app import create_app
...
def test_login_success(client):
    response = client.post("/login", json={"username": "emperor", "password": "fugazi"})
...
def skip_test_delete_strategy(client, access_token):
    ...
    client.post(
        "/create_strategy",
        json={"broker": "test_broker", "strategy_name": "test_strategy"},
...
def skip_test_get_brokers_strategies(client, access_token):
    response = client.get(
        "/get_brokers_strategies", headers={"Authorization": f"Bearer {access_token}"}
    )
    assert response.status_code == 200
    data = response.get_json()
