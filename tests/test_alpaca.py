...
class TestAlpacaBroker(BaseTest):
    ...
    async def test_place_order(self, mock_post):
        ...
        await self.broker.connect()
        order_info = await self.broker._place_order(
            "AAPL", 10, "buy", price=150.00, order_type="limit"
        )
        ...
        balance = await self.session.execute(
            select(Balance).filter_by(broker="Alpaca", strategy="default")
        )
        balance = balance.scalar_one()
        assert balance is not None
        assert balance.total_balance == 10000.0 + (
            10 * 155.00
        )  # Assuming balance reflects executed trade
    ...
    async def test_place_option_order(self, mock_get):
        ...
        async def place_option_order_test(mock_post):
            ...
            await self.broker.connect()
            order_info = await self.broker._place_option_order(
                "AAPL", 1, "buy", "call", 150.00, "2024-12-20", price=10.00
            )
