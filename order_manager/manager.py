...
class OrderManager:
    ...
    async def reconcile_order(self, order):
        ...
        if order.timestamp < stale_threshold and order.status not in [
            "filled",
            "canceled",
        ]:
            try:
                logger.info(
                    f"Marking order {order.id} as stale", extra={"order_id": order.id}
                )
                await self.db_manager.update_trade_status(order.id, "stale")
                return  # Exit early if the order is stale
            except Exception as e:
                logger.error(
                    f"Error marking order {order.id} as stale", extra={"error": str(e)}
                )
                return
        ...
        if order.broker_id is None:
            ...
            logger.info(
                f"Marking order {order.id} as stale, missing broker_id",
        ...
        if filled:
            try:
                ...
                logger.error(
                    f"Error reconciling order {order.id}", extra={"error": str(e)}
                )
