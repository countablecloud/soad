...
from utils.config import (
    create_api_database_engine,
    create_database_engine,
    initialize_brokers,
    initialize_brokers_and_strategies,
    initialize_database,
    parse_config,
)
from utils.logger import logger  # Import the logger
from utils.utils import is_futures_market_open, is_market_open
...
async def start_trading_system(config_path):
    ...
    try:
        ...
        logger.error(
            "Failed to parse configuration", extra={"error": str(e)}, exc_info=True
        )
        return
    ...
    rebalance_intervals = {
        s: timedelta(minutes=strategies[s].rebalance_interval_minutes)
        for s in strategies
    }
    ...
    while datetime.now() < end_time:
        ...
        for strategy_name, strategy in strategies.items():
            if (
                now - last_rebalances[strategy_name]
                >= rebalance_intervals[strategy_name]
            ):
                try:
                    await strategy.rebalance()
                except Exception as e:
                    logger.error(
                        f"Error during rebalancing strategy {strategy_name}",
                        extra={"error": str(e)},
                        exc_info=True,
                    )
                    brokers, strategies = await initialize_brokers_and_strategies(
                        config
                    )
        ...
        #            logger.info(f'Strategy {strategy_name} rebalanced successfully',
        #               extra={'time': now})
        #    except Exception as e:
...
async def start_api_server(config_path=None, local_testing=False):
    logger.info(
        "Starting API server",
        extra={"config_path": config_path, "local_testing": local_testing},
    )
    ...
    if config_path is None:
        ...
        try:
            ...
            logger.error(
                "Failed to parse configuration for API server",
                extra={"error": str(e)},
                exc_info=True,
            )
            return
    ...
    async_db_url = os.environ.get(
        "ASYNC_DATABASE_URL", "sqlite+aiosqlite:///default_trading_system.db"
    )
    ...
    try:
        ...
        logger.error(
            "Failed to start API server", extra={"error": str(e)}, exc_info=True
        )
...
async def start_order_manager(config_path):
    ...
    while True:
        try:
            ...
            logger.error(
                "Failed to start order manager, trying to initialize brokers again",
                extra={"error": str(e)},
                exc_info=True,
            )
            brokers = initialize_brokers(config)
...
async def start_sync_worker(config_path):
    ...
    while True:
        try:
            ...
            logger.error(
                "Failed to start sync worker, trying to initialize brokers again",
                extra={"error": str(e)},
                exc_info=True,
            )
            brokers = initialize_brokers(config)
...
async def main():
    parser = argparse.ArgumentParser(
        description=(
            "Run trading strategies, start API server, "
            "or start sync worker based on YAML configuration."
        )
    )
    parser.add_argument(
        "--mode",
        choices=["trade", "api", "sync", "manager"],
        required=True,
        help='Mode to run the system in: "trade", "api", or "sync"',
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--local_testing",
        action="store_true",
        help="Run API server with local testing configuration.",
    )
    ...
    if args.mode == "trade":
        ...
        await start_api_server(
            config_path=args.config, local_testing=args.local_testing
        )
    elif args.mode == "sync":
        if not args.config:
        ...
        try:
            ...
            logger.error(
                "Error in order manager", extra={"error": str(e)}, exc_info=True
            )
            await start_order_manager(args.config)
